...
# Other divvyd's trusting this validator need this key
[validator_keys]
{all_validator_keys}
...
def manifest_create():
    ...
    to_run = ['python', ARGS.divvy_home + '/bin/python/Manifest.py', 'create']
...
def sign_manifest(seq, validation_pk, master_secret):
    ...
    to_run = ['python', ARGS.divvy_home + '/bin/python/Manifest.py', 'sign',
...
def get_divvy_exe():
    """Find the divvyd executable"""
    prefix = ARGS.divvy_home + '/build/'
    exe = ['divvyd', 'DivvyD.exe']
...
class DivvydServer(object):
    ...
    def startup(self):
        if ARGS.verbose:
            print('starting divvyd:' + self.config_file)
...
@contextmanager
def divvyd_server(config_file, server_out=os.devnull):
    """Start a divvy server"""
    try:
        server = None
        server = DivvydServer(ARGS.divvy_exe, config_file, server_out)
        server.startup()
        yield server
    finally:
        if server:
...
def pause_server(server, config_file):
    """Shutdown and then restart a divvy server"""
    try:
        server.shutdown()
...
def new_config_ephemeral_key(
    ...
    config_file = config_root + '/divvyd.cfg'
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=('Root directory of the divvy repo'), )
    parser.add_argument('--num_validators', '-n',
                        default=2,
                        help=('Number of validators'), )
    parser.add_argument('--conf', '-c', help=('divvyd config file'), )
...
def get_configs(manifest_seq):
    global ARGS
    ARGS.divvy_home = os.path.expanduser(ARGS.divvy_home)

    n = int(ARGS.num_validators)
    if n<2:
        raise ValueError(
            'Need at least 2 divvyd servers. Specified: {}'.format(n))
    config_root = ARGS.out
    ARGS.divvy_exe = get_divvy_exe()
    if not ARGS.divvy_exe:
        raise ValueError('No Exe Found')

    if ARGS.existing:
        return [
            os.path.abspath('{}/validator_{}/divvyd.cfg'.format(config_root, i))
            for i in range(n)
        ]
    ...
    with divvyd_server(initial_config) as server:
    ...
    for i, (m, v, s) in enumerate(zip(manifests, validations, signed_manifests)):
        ...
        config_files.append('{}/divvyd.cfg'.format(this_validator_dir))
...
def run_main():
    ...
    if len(config_files) <= 1:
        ...
        return
    with contextlib.nested(*(divvyd_server(c, os.path.dirname(c)+'/log.txt')
