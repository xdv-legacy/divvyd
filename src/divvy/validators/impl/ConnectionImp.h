...
#include <divvy/validators/Connection.h>
...
private:
    ...
    using Item = std::pair<LedgerHash, DivvyAddress>;
    ...
    boost::container::flat_map<DivvyAddress, Source> sources_;
    boost::container::flat_set<DivvyAddress> good_;
...
public:
    ...
    onLedger (LedgerHash const& ledger)
    {
        ...
        auto item = items_.lower_bound(
            std::make_pair(ledger, DivvyAddress()));
