...
#include <divvy/app/ledger/tests/common_ledger.h>
...
#include <divvy/rpc/impl/DivvyPathFind.h>
...
// Create an account represented by public DivvyAddress and private
// DivvyAddress
TestAccount
createAccount(std::string const& passphrase, KeyType keyType)
{
    DivvyAddress const seed
        = DivvyAddress::createSeedGeneric(passphrase);
...
Ledger::pointer const& ledger, bool sign)
{
    ...
    auto cache = std::make_shared<DivvyLineCache>(ledger);
    STPathSet pathSet;
    ...
    Ledger::pointer const& ledger, bool sign)
{
    ...
    tx_json[jss::Flags] = 0;    // tfClearNoDivvy;
    STTx tx = parseTransaction(from, tx_json, sign);
    applyTransaction(ledger, tx, sign);
}
    ...
    boost::optional<Json::Value> contextPaths)
{
    ...
    auto result = divvyPathFind(cache, src.pk, dest.pk, saDstAmount,
        ledger, jvSrcCurrencies, contextPaths, level);
    if(!result.first)
        throw std::runtime_error(
        "divvyPathFind find failed");

    return result.second;
}

std::shared_ptr<SLE const>
getLedgerEntryDivvyState(Ledger::pointer ledger,
    TestAccount const& account1, TestAccount const& account2,
        Currency currency)
{
    auto uNodeIndex = getDivvyStateIndex(
...
verifyBalance(Ledger::pointer ledger, TestAccount const& account,
    Amount const& amount)
{
    auto const sle = getLedgerEntryDivvyState(ledger, account,
