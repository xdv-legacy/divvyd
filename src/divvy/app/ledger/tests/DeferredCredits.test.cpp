    ...
    */
    void testSelfFunding ()
    {
        ...
        payWithPath (snd, rcv, "USD", "4", ledger, path,
                     tfNoDivvyDirect | tfPartialPayment);
    ...
    void testSubtractCredits ()
    {
        ...
        divvy::Currency const usd (to_currency ("USD"));
        divvy::Issue const issue (usd, gw1Acc);
...

        {
            ...
            les.divvyCredit (gw1Acc, aliceAcc, toCredit);
            expect (les.accountHolds (aliceAcc, usd, gw1Acc, fhIGNORE_FREEZE) ==
                    startingAmount + toCredit);

            les.divvyCredit (aliceAcc, gw1Acc, toDebit);
            expect (les.accountHolds (aliceAcc, usd, gw1Acc, fhIGNORE_FREEZE) ==
                    startingAmount + toCredit - toDebit);
        }
        ...
        {
            // divvyCredit, w/ FT & ScopedDeferCredits
            LedgerEntrySet les (ledger, tapNONE);
...
BEAST_DEFINE_TESTSUITE (DeferredCredits, ledger, divvy);
