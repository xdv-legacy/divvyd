...
{
    DivvyAddress const seed = DivvyAddress::createSeedGeneric (passphrase);
...
std::vector<DivvyState::pointer> getDivvyStates (
    TestLedger& ledger, UserAccount const& acct, UserAccount const& peer)
{
    ...
    forEachItem(*ledger.openLedger(), acct.getID(), getApp().getSLECache(),
            ...
            std::shared_ptr<SLE const> const& sleCur)
        {
            ...
            DivvyState::pointer const state (
                DivvyState::makeItem (acct.getID(), sleCur));
