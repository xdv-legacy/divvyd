...
#include <divvy/app/tx/tests/common_transactor.h>
...
{
    static std::uint64_t const xdv = std::mega::num;
...
//------------------------------------------------------------------------------
    ...
    void test_singleSig (KeyType kType)
    {
        ...
        TestLedger ledger (100000*xdv, master, *this);
...
//------------------------------------------------------------------------------
    ...
    void test_signerListSet (KeyType kType)
    {
        ...
        // User accounts
        UserAccount alice (kType, "alice");
        std::uint64_t aliceBalance = 1000*xdv;
        ...
        payInDrops (ledger, master, becky, 1000*xdv);

        UserAccount cheri (kType, "cheri");
        payInDrops (ledger, master, cheri, 1000*xdv);

        UserAccount daria (kType, "daria");
        payInDrops (ledger, master, daria, 1000*xdv);
        {
...
//------------------------------------------------------------------------------
    ...
    void test_heterogeneousSigners (KeyType kType)
    {
        ...
        payInDrops (ledger, master, edith, 1000*xdv);
...
//------------------------------------------------------------------------------
    ...
    void test_twoLevel(KeyType kType)
    {
        ...
        payInDrops (ledger, master, alice, 1000*xdv);
        aliceBalanceCheck (0);
        alice.setRegKey (ledger, kType, "alie");
        aliceBalanceCheck (stdFee);
        alice.disableMaster (ledger, true);
        ...
        payInDrops (ledger, master, freda, 1000*xdv);

        UserAccount ginny (kType, "ginny");
        payInDrops (ledger, master, ginny, 1000*xdv);

        UserAccount helen (kType, "helen");
        payInDrops (ledger, master, helen, 1000*xdv);

        UserAccount irena (kType, "irena");
        payInDrops (ledger, master, irena, 1000*xdv);
...
//------------------------------------------------------------------------------
    ...
    void test_txTypes(KeyType kType)
    {
        ...
        // 2-level multi-sign a ttREGULAR_KEY_SET.
        {
            ...
            {
                DivvyAddress const seed =
                    DivvyAddress::createSeedGeneric("BadNewsBears");
            ...
            {
                DivvyAddress const seed =
                    DivvyAddress::createSeedGeneric("alie");
                KeyPair regular = generateKeysFromSeed (kType, seed);
                STTx tx = getSetRegularKeyTx (
                    alice, regular.publicKey.getAccountID());
        ...
        // We need a Currency and Issue.  The next tests use non-XDV.
        divvy::Currency const knuts = to_currency("KNT");
        UserAccount gringots (kType, "Gringots Wizarding Bank");
        payInDrops (ledger, master, gringots, 10000*xdv);
        ...
        // 2-level multi-sign a ttTRUST_SET transaction.
        {
            ...
            {
                std::vector<DivvyState::pointer> states =
                    getDivvyStates (ledger, alice, gringots);
        ...
        // 2-level multi-sign ttOFFER_CREATE and ttOFFER_CANCEL transactions.
        {
            ...
            // alice has 50 knuts.  She'll offer to trade them for 50 XDV.
            {
...
BEAST_DEFINE_TESTSUITE(MultiSign,divvy_app,divvy);
