...
private:
    ...
    std::map<DivvyAddress, ClusterNodeStatus> m_clusterNodes;
...
public:
    ...
    void nodeAddPublic (DivvyAddress const& naNodePublic, ValidatorSource vsWhy, std::string const& strComment);

    // Queue a domain for a single attempt fetch a divvy.txt.
    ...
    void nodeRemovePublic (DivvyAddress const& naNodePublic);
    ...
    bool nodeInUNL (DivvyAddress const& naNodePublic);

    bool nodeInCluster (DivvyAddress const& naNodePublic);
    bool nodeInCluster (DivvyAddress const& naNodePublic, std::string& name);

    bool nodeUpdate (DivvyAddress const& naNodePublic, ClusterNodeStatus const& cnsStatus);

    std::map<DivvyAddress, ClusterNodeStatus> getClusterStatus();
...
private:
    ...
    // Try to process the next fetch of a divvy.txt.
    void fetchNext();
    ...
    // Get the divvy.txt and process it.
    void fetchProcess (std::string strDomain);

    // Process IniFileSections [validators_url].
    void getValidatorsUrl (DivvyAddress const& naNodePublic,
    ...
    void getIpsUrl (DivvyAddress const& naNodePublic, IniFileSections secSite);


    // Given a IniFileSections with IPs, parse and persist it for a validator.
    bool responseIps (std::string const& strSite, DivvyAddress const& naNodePublic, const boost::system::error_code& err, int iStatus, std::string const& strIpsFile);;

    // After fetching a divvy.txt from a web site, given a IniFileSections with validators, parse and persist it.
    bool responseValidators (std::string const& strValidatorsUrl, DivvyAddress const& naNodePublic, IniFileSections secSite, std::string const& strSite, const boost::system::error_code& err, int iStatus, std::string const& strValidatorsFile);
    ...
    void processIps (std::string const& strSite, DivvyAddress const& naNodePublic, IniFileSections::mapped_type* pmtVecStrIps);
    ...
    int processValidators (std::string const& strSite, std::string const& strValidatorsSrc, DivvyAddress const& naNodePublic, ValidatorSource vsWhy, IniFileSections::mapped_type* pmtVecStrValidators);

    // Process a divvy.txt.
    void processFile (std::string const& strDomain, DivvyAddress const& naNodePublic, IniFileSections secSite);
    ...
    bool getSeedNodes (DivvyAddress const& naNodePublic, seedNode& dstSeedNode);
...
// Add a trusted node.  Called by RPC or other source.
void UniqueNodeListImp::nodeAddPublic (DivvyAddress const& naNodePublic, ValidatorSource vsWhy, std::string const& strComment)
{
    seedNode    snCurrent;
...
void UniqueNodeListImp::nodeRemovePublic (DivvyAddress const& naNodePublic)
{
    {
...
bool UniqueNodeListImp::nodeInUNL (DivvyAddress const& naNodePublic)
{
...
bool UniqueNodeListImp::nodeInCluster (DivvyAddress const& naNodePublic)
{
    ScopedUNLLockType sl (mUNLLock);
...
bool UniqueNodeListImp::nodeInCluster (DivvyAddress const& naNodePublic, std::string& name)
{
    ScopedUNLLockType sl (mUNLLock);
    std::map<DivvyAddress, ClusterNodeStatus>::iterator it = m_clusterNodes.find (naNodePublic);
...
bool UniqueNodeListImp::nodeUpdate (DivvyAddress const& naNodePublic, ClusterNodeStatus const& cnsStatus)
{
    ScopedUNLLockType sl (mUNLLock);
...
std::map<DivvyAddress, ClusterNodeStatus>
UniqueNodeListImp::getClusterStatus()
{
    std::map<DivvyAddress, ClusterNodeStatus> ret;
    {
        ScopedUNLLockType sl (mUNLLock);
        ret = m_clusterNodes;
    }
...
std::uint32_t UniqueNodeListImp::getClusterFee()
{
    ...
    std::vector<std::uint32_t> fees;
    {
        ScopedUNLLockType sl (mUNLLock);
        {
            for (std::map<DivvyAddress, ClusterNodeStatus>::iterator it = m_clusterNodes.begin(),
...
void UniqueNodeListImp::nodeBootstrap()
{
    ...
    {
        DivvyAddress   naInvalid;  // Don't want a referrer on added entries.
...
void UniqueNodeListImp::trustedLoad()
{
    ...
    for (auto const& c : getConfig ().CLUSTER_NODES)
    {
        ...
        {
            DivvyAddress a = DivvyAddress::createNodePublic (match[1]);
...
// Process IniFileSections [validators_url].
void UniqueNodeListImp::getValidatorsUrl (DivvyAddress const& naNodePublic,
    IniFileSections secSite)
{
...
// Process IniFileSections [ips_url].
// If we have a IniFileSections with a single entry, fetch the url and process it.
void UniqueNodeListImp::getIpsUrl (DivvyAddress const& naNodePublic, IniFileSections secSite)
{
...
// Given a IniFileSections with IPs, parse and persist it for a validator.
bool UniqueNodeListImp::responseIps (std::string const& strSite, DivvyAddress const& naNodePublic, const boost::system::error_code& err, int iStatus, std::string const& strIpsFile)
{
    bool    bReject = !err && iStatus != 200;
...
// After fetching a divvy.txt from a web site, given a IniFileSections with validators, parse and persist it.
bool UniqueNodeListImp::responseValidators (std::string const& strValidatorsUrl, DivvyAddress const& naNodePublic, IniFileSections secSite, std::string const& strSite, const boost::system::error_code& err, int iStatus, std::string const& strValidatorsFile)
{
    bool    bReject = !err && iStatus != 200;
...
// --> strSite: source of the IPs (for debugging)
// --> naNodePublic: public key of the validating node.
void UniqueNodeListImp::processIps (std::string const& strSite, DivvyAddress const& naNodePublic, IniFileSections::mapped_type* pmtVecStrIps)
{
...
// --> naNodePublic: remote source public key - not valid for local
// --> vsWhy: reason for adding validator to SeedDomains or SeedNodes.
int UniqueNodeListImp::processValidators (std::string const& strSite, std::string const& strValidatorsSrc, DivvyAddress const& naNodePublic, ValidatorSource vsWhy, IniFileSections::mapped_type* pmtVecStrValidators)
{
...
// Process a divvy.txt.
void UniqueNodeListImp::processFile (std::string const& strDomain, DivvyAddress const& naNodePublic, IniFileSections secSite)
{
    //
    // Process Validators
    //
...
// Retrieve a SeedNode from DB.
bool UniqueNodeListImp::getSeedNodes (DivvyAddress const& naNodePublic, seedNode& dstSeedNode)
{
    std::string strSql =
            str (boost::format (
