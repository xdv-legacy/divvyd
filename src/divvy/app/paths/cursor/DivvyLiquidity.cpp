    ...
    std::uint64_t& uRateMax)
{
    ...
    if (uQualityIn >= uQualityOut)
    {
        ...
        WriteLog (lsTRACE, DivvyCalc) << "divvyLiquidity: No fees";
    ...
    else
    {
        ...
        WriteLog (lsTRACE, DivvyCalc) << "divvyLiquidity: Fee";
    ...
    SField const& sfHigh)
{
    ...
    {
        SLE::pointer sleDivvyState (ledger.entryCache (ltRIPPLE_STATE,
            getDivvyStateIndex (destination, source, currency)));

        // we should be able to assert(sleDivvyState) here

        if (sleDivvyState)
        {
            auto const& sfField = destination < source ? sfLow : sfHigh;

            uQuality = sleDivvyState->isFieldPresent (sfField)
                ? sleDivvyState->getFieldU32 (sfField)
                : QUALITY_ONE;
...
quality_in (
    ...
    Currency const& currency)
{
    return divvyQuality (ledger, uToAccountID, uFromAccountID, currency,
