...
bool PathRequest::isValid (DivvyLineCache::ref crCache)
{
    ScopedLockType sl (mLock);
        ...
        else
        {
            ...
            else
            {
                bool const disallowXDV (
                    asDst->sle().getFlags() & lsfDisallowXDV);
...
Json::Value PathRequest::doUpdate (DivvyLineCache::ref cache, bool fast)
{
    ...
    for (auto const& currIssuer: sourceCurrencies)
    {
        ...
        if (valid)
        {
            ...
            {
                m_journal.debug << iIdentifier << " divvyCalc returns "
                    << transHuman (rc.result ());
            }
        }
        else
