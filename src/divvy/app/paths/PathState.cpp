...
void PathState::reset(STAmount const& in, STAmount const& out)
{
    ...
    CondLog (outAct() >= outReq(), lsWARNING, DivvyCalc)
    ...
    AccountID const& issuer)    // --> Delivering this issuer.
{
    TER resultCode = tesSUCCESS;
     ...
     WriteLog (lsTRACE, DivvyCalc) << "pushImpliedNodes>" <<
         " " << account <<
         " " << currency <<
         " " << issuer;
    ...
    AccountID const& issuer)     // If not specified, default to previous.
{
    ...
    if (iType & ~STPathElement::typeAll)
    {
        ...
        WriteLog (lsDEBUG, DivvyCalc) << "pushNode: bad bits.";
        resultCode = temBAD_PATH;
    }
    else if (hasIssuer && isXDV (node.issue_))
    {
        WriteLog (lsDEBUG, DivvyCalc) << "pushNode: issuer specified for XDV.";

        resultCode = temBAD_PATH;
    }
    else if (hasIssuer && !issuer)
    {
        WriteLog (lsDEBUG, DivvyCalc) << "pushNode: specified bad issuer.";
    ...
    else if (hasAccount)
    {
        ...
        if (resultCode == tesSUCCESS && !nodes_.empty ())
        {
            ...
            {
                auto sleDivvyState = lesEntries_->entryCache (
                    ltRIPPLE_STATE,
                    getDivvyStateIndex (
                        backNode.account_,
                        node.account_,
                ...
                if (!sleDivvyState)
                {
                    ...
                    WriteLog (lsTRACE, DivvyCalc) << getJson ();
                ...
                else
                {
                    ...
                    if (!sleBck)
                    {
                        ...
                        resultCode   = terNO_ACCOUNT;
                             ...
                             sleDivvyState->getFieldAmount(sfBalance) == zero)
                    {
                        WriteLog (lsWARNING, DivvyCalc)
                                << "pushNode: delay: can't receive IOUs from "
                                << "issuer without auth.";
    ...
    else
    {
        ...
        else if (backNode.issue_ == node.issue_)
        {
            ...
            WriteLog (lsTRACE, DivvyCalc) << "pushNode: imply for offer.";
    ...
    WriteLog (lsTRACE, DivvyCalc) << "pushNode< : " << transToken (resultCode);
    return resultCode;
}
    ...
    AccountID const& uSenderID)
{
    ...
    for (auto & speElement: spSourcePath)
    {
        if (terStatus == tesSUCCESS)
        {
            WriteLog (lsTRACE, DivvyCalc) << "expandPath: element in path";
            terStatus = pushNode (
    ...
    Currency const& currency)
{
    ...
    SLE::pointer sleIn = lesEntries_->entryCache (ltRIPPLE_STATE,
        getDivvyStateIndex (firstAccount, secondAccount, currency));
    SLE::pointer sleOut = lesEntries_->entryCache (ltRIPPLE_STATE,
        getDivvyStateIndex (secondAccount, thirdAccount, currency));
