...
Making a payment in the divvy network is about finding the cheapest path between source and destination.
...
When someone accepts payment they list their specific payment terms, "what must happen before the payment goes off."  That can be done completely in the Divvy Network.  Normally a payment on the Divvy net can be completely settled there.  When the ledger closes, the terms are met, or they are not met.
...
In contrast, a gateway is in the business of redeeming value on the Divvy Network for value off the Ripnet.  A gateway is in the business of issuing balances.

Bitstamp is a gateway - you send 'em cash, they give you a divvy balance and vice versa.  There's no promise or forwarding for a transaction.

A bridge is a facility that allows payments to be made from the Ripnet to off the Ripnet.

Suppose I'm on the Divvy network and want to send value to the bitcoin network.  See:  https://xdv.io/wiki/Outbound_Bridge_Payments
https://xdv.io/wiki/Services_API
...
https://xdv.io/wiki/Transaction_Format#Payment_.280.29
...
1. Finding liquidity - "path finding"
  ...
  * filtering by liquidity.
    ...
    * This routine is called DivvyCalc::divvyCalc.
 ...
 * The routine used to compute liquidity and ledger change is also called DivvyCalc::divvyCalc.
