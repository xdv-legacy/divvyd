...
#include <divvy/rpc/impl/KeypairForSignature.h>
...
{
private:
    DivvyAddress const* const signingForAcctID_;
    DivvyAddress const* const multiSigningAcctID_;
    DivvyAddress* const multiSignPublicKey_;
...
public:
    ...
    SigningForParams (
        DivvyAddress const& signingForAcctID,
        DivvyAddress const& multiSigningAcctID,
        DivvyAddress& multiSignPublicKey,
        Blob& multiSignature)
    ...
    DivvyAddress const& getSigningFor ()
    {
        return *signingForAcctID_;
    }

    DivvyAddress const& getSigner ()
    {
        return *multiSigningAcctID_;
    }

    void setPublicKey (DivvyAddress const& multiSignPublicKey)
    {
        *multiSignPublicKey_ = multiSignPublicKey;
    }
...
void TxnSignApiFacade::snapshotAccountState (DivvyAddress const& accountID)
{
    ...
    STPath& fullLiquidityPath) const
{
    ...
    auto cache = std::make_shared<DivvyLineCache> (ledger_);
    return divvy::findPathsForOneIssuer (
        cache,
...
error_code_i acctMatchesPubKey (
    ...
    DivvyAddress const& accountID,
    DivvyAddress const& publicKey)
{
...
static Json::Value checkPayment(
    ...
    Json::Value& tx_json,
    DivvyAddress const& raSrcAddressID,
...
//
// Returns a pair<Json::Value, DivvyAddress>.  The Json::Value is non-empty
// and contains as error if there was an error.  The returned DivvyAddress
...
static std::pair<Json::Value, DivvyAddress>
checkTxJsonFields (
    ...
    bool const verify)
{
    std::pair<Json::Value, DivvyAddress> ret;
    ...
    SigningForParams& signingArgs)
{
    ...
    DivvyAddress const raSrcAddressID = std::move (txJsonResult.second);
...
void insertMultiSigners (
    Json::Value& jvResult,
    DivvyAddress const& signingForAcctID,
    DivvyAddress const& signingAcctID,
    DivvyAddress const& accountPublic,
    ...
    Role role)
{
    ...
    // Turn the signer's account into a DivvyAddress for multi-sign.
    DivvyAddress multiSignAddrID;
    ...
    // Turn the signing_for account into a DivvyAddress for multi-sign.
    DivvyAddress multiSignForAddrID;
    ...
    Role role)
{
    ...
    for (STObject& signingFor : multiSigners)
    {
        ...
        DivvyAddress const signingForAcct =
