...
Json::Value doDivvyPathFind (RPC::Context& context)
{
    ...
    {
        context.loadType = Resource::feeHighBurdenRPC;
        DivvyLineCache::pointer cache;

        if (lpLedger)
        {
            // The caller specified a ledger
            lpLedger = std::make_shared<Ledger> (std::ref (*lpLedger), false);
            cache = std::make_shared<DivvyLineCache>(lpLedger);
        }
        else
        {
        ...
        auto pathFindResult = divvyPathFind(cache, raSrc, raDst, saDstAmount, 
    ...
    WriteLog (lsDEBUG, RPCHandler)
            << boost::str (boost::format ("divvy_path_find< %s")
                           % jvResult);
...
Json::Value
buildSrcCurrencies(DivvyAddress const& raSrc, DivvyLineCache::pointer const& cache)
{
...
divvyPathFind(DivvyLineCache::pointer const& cache, 
  DivvyAddress const& raSrc, DivvyAddress const& raDst,
    ...
    for (unsigned int i = 0; i != jvSrcCurrencies.size(); ++i)
    {
        ...
        if (contextPaths)
        {
            ...
            else
            {
                ...
                WriteLog(lsTRACE, RPCHandler) << "divvy_path_find: Paths: " <<
        ...
        if (!valid)
        {
            WriteLog(lsWARNING, RPCHandler)
                << "divvy_path_find: No paths found.";
        }
        else
        {
            auto& issuer =
